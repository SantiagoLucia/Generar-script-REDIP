...
def crear_scripts_sql(df):
    ...
    with engine.connect() as conn:
        ...
        query = sqlalchemy.text(
            """SELECT ID_CIRCUNSCRIPCION FROM 
        ...
        query = sqlalchemy.text(
            """SELECT ID_KODAK_CIRCUNSCRIPCION FROM RCE_GED.SYS_KODAK_CIRCUNSCRIPCION 
        ...
        for _, row in df.iterrows():
            ...
            if int(reg) in (3, 5, 6):
                if flag_encabezado_kodak == 1:
                    ...
                    escribir_archivo(
                        nombre_archivo_k, "encabezado_kodak", "w+", cant2, k_ini
                    )

                escribir_archivo(
                    nombre_archivo_k, "insert_kodak", "a", reg, renaper, id_cir_str
                )
